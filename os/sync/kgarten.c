...
/***************************************************************/	
	...
	{ 
		...
		do{ 
			...
			if (thr->kg->space) { /* If there is space for entering a child*/  
				   ...
				   flag = 1;         /* flag enabled*/ 
					if (thr->kg->space) /* If there is free space, send signal to other children */ 
	...
	{ 
		...
		/* LOCK */ 
		pthread_mutex_lock(&thr->kg->mutex); 
		--(thr->kg->vc);					/*decrease the number of children by 1*/
		thr->kg->space++;		/*So increase the space*/
		if(thr->kg->space > thr->kg->ratio) 
			pthread_cond_broadcast(&thr->kg->condVar);	/*if there is enough space teacher is allowed to leave */
		else 
	...
	{ 
		...
		pthread_mutex_lock(&thr->kg->mutex); 
		while(thr->kg->space < thr->kg->ratio) pthread_cond_wait(&thr->kg->condVar, &thr->kg->mutex);	//wait until teacher can leave  
